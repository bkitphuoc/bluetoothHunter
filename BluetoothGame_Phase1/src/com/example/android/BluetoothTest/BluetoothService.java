...
//    public static boolean isServer = false;
    ...
    public long startTime_t = 0L;
    public long milisecondsCounter = 0L;
    ...
    public synchronized void connect(BluetoothDevice device, boolean secure) {
    	//em thinh them code
    	bluetoothDeviceBackup = mAdapter.getRemoteDevice(device.getAddress());
    	secureBackUp = secure;
...
//        }
     ...
     */
    private void connectionLostAsServer() {
        // Send a failure message back to the Activity
        Message msg = mHandler.obtainMessage(BluetoothTest.MESSAGE_TOAST);
        Bundle bundle = new Bundle();
        bundle.putString(BluetoothTest.TOAST, "Device connection was lost server");
        msg.setData(bundle);
        mHandler.sendMessage(msg);
    ...
    private void connectionLostAsClient() {
        // Send a failure message back to the Activity
        Message msg = mHandler.obtainMessage(BluetoothTest.MESSAGE_TOAST);
        Bundle bundle = new Bundle();
        bundle.putString(BluetoothTest.TOAST, "Device connection was lost client");
        msg.setData(bundle);
        mHandler.sendMessage(msg);

        // Start the service over to restart listening mode
        connect(bluetoothDeviceBackup, secureBackUp);
    }
            ...
            while(true){
            	try{
            		...
            		if(mmSocket.isConnected() == true)
            			break;
            		startTime_t = SystemClock.uptimeMillis();
            		while(true){
            			milisecondsCounter = SystemClock.uptimeMillis();
            			int seconds = (int) (milisecondsCounter - startTime_t) / 1000;
            			if(seconds == 5)
            				break;
            		}
        ...
        public void run() {
            ...
            while (true) {
                try {
                    ...
                    if(readMessage.equals("I confirm you press back")){
                    	Log.i(TAG,"nhan:  Iconfirm ================>");
                    	disableReConnect = true;
